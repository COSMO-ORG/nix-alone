  ...
  SUBROUTINE calc_hn_density(nvec, ivstart, ivend, rho_hn, uv, t_a)
    ...
    INTEGER :: &
     ...
     REAL (KIND = wp) :: &
                  ...
                  zrho_hn_min        ! adjusted lower bound of new snow density      (kg/m**3)
    ...
    ! -------------------------
    ! Parameterisation - TERRA
    ! -------------------------

            rho_hn(i) = rho_hn_min + (rho_hn_max - rho_hn_min) * ((t_a(i)-258.15_wp) / (t0_melt-258.15_wp))**2
!      ! Increase lower bound of new snow density due to compression by wind - analog to TERRA
!      zrho_hn_min = rho_hn_min * MIN(2._wp, 1._wp + 0.125_wp*MAX(0._wp, uv(i)-4._wp))
!
!      ! New snow density formulation from TERRA.
!      rho_hn(i) = zrho_hn_min + (rho_hn_max - zrho_hn_min) * ((t_a(i)-258.15_wp) / (t0_melt-258.15_wp))**2
!
!      ! Apply limits FIXME: TERRA also adjusts the lower bound for graupel.
         ENDIF
  ...
  SUBROUTINE calc_hn_amounts(nvec, ivstart, ivend, hn_sn, zsnow_rate, dt)
    ...
    ! ------------------------------------------------------------------------------     
      ...
      DO i = ivstart, ivend
         ...
         hn_sn(i) = hn_sn(i) + (MAX(0.0_wp,zsnow_rate(i))*dt) !/rho_w)     FIXME! The commented out division by rho_w should be sorted out.
...
! ============================================================================
  ...
  SUBROUTINE update_nix_state(nvec, ivstart, ivend, top, ke_snow , &
    ...
    DO i = ivstart, ivend
     ...
     ! Update snow surface temperature with nodal temperatures
