...
MODULE mo_nix_water_transport
  ...
  USE mo_nix_constants,           ONLY: rho_i, eps_div
...
! ============================================================================
  ...
  SUBROUTINE surface_mass_flux(nvec, ivstart, ivend, ke_soil, ke_snow  , &
    ...
    DO i = ivstart, ivend
        ...
        ! Assign latent heat flux to local variable to avoid nullification 
  ...
  SUBROUTINE water_transport(nvec, ivstart, ivend, ke_soil, ke_snow              , &
    ...
    DO i = ivstart, ivend
      ...
      DO ksn = 1, top(i), 1
        ...
        dtheta_w_sub_wtr = (hcap_sn(i,ksn) * rho_sn(i,ksn)) * (1.0_wp / lh_f ) * (1.0_wp / rho_w) * &
                           & MAX(0.0_wp, (t0_melt - t_sn(i,ksn)))
